...
1. Follow the step 4 `Train the agent` in `Tennis_Train.ipynb` to train the agent. Training algorithm is DDPG with prioritized replay buffer and soft update.
2. Training is expected to be done in 1000 episodes.
3. The trained weights will be saved in `checkpoint_actor.pth` and `checkpoint_critic.pth`.
4. Follow the steps in `Tennis_Test.ipynb` to watch a trained agent.
